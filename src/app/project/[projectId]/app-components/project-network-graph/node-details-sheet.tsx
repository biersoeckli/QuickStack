...
function AppStatusActions({ app, role }: { app: AppExtendedModel; role?: RolePermissionEnum }) {
    ...
    const canStart = ['ERROR', 'UNKNOWN', 'SHUTDOWN', 'SHUTTING_DOWN'].includes(deploymentStatus);
    const canStop = ['BUILDING', 'DEPLOYED', 'ERROR', 'UNKNOWN', 'DEPLOYING'].includes(deploymentStatus);

    return (
        <div className="flex items-center rounded-md bg-emerald-500/10 px-2 py-1">
            <PodStatusIndicator appId={app.id} showLabel />
            {canManage && <DropdownMenu>
                <DropdownMenuTrigger asChild><Button variant="ghost" size="icon" className="ml-1 size-5"><ChevronDown className="size-3.5" /><span className="sr-only">App actions</span></Button></DropdownMenuTrigger>
                <DropdownMenuContent align="end">
                    <DropdownMenuItem disabled={!appSourceIsConfigured} onSelect={() => void Toast.fromAction(() => deploy(app.id))}><Rocket />Deploy</DropdownMenuItem>
                    {app.appType === 'APP' && (app.sourceType === 'GIT' || app.sourceType === 'GIT_SSH') && <DropdownMenuItem disabled={!appSourceIsConfigured} onSelect={() => void Toast.fromAction(() => deploy(app.id, true))}><Hammer />Rebuild</DropdownMenuItem>}
                    <DropdownMenuItem disabled={!canStart || !appSourceIsConfigured} onSelect={() => void Toast.fromAction(() => startApp(app.id))}><Play />Start</DropdownMenuItem>
                    <DropdownMenuItem disabled={!canStop || !appSourceIsConfigured} onSelect={() => void Toast.fromAction(() => stopApp(app.id))}><Square />Stop</DropdownMenuItem>
                </DropdownMenuContent>
            </DropdownMenu>}
        </div>
    );
...
export function NodeDetailsSheet({
    ...
    const connectionsContent = (
        <div className="space-y-2">
            ...
            ) : <ItemGroup>{connections.map(connection => (
                <Item key={connection.id} variant="outline" size="sm">
                    <ItemMedia variant="icon" className={connection.name === 'Internet' ? 'bg-violet-500/10 text-violet-600' : 'bg-qs-500/10 text-qs-600'}><Globe2 className="size-4" /></ItemMedia>
                    <ItemContent><ItemTitle>{connection.name}</ItemTitle><ItemDescription>{connection.direction}{connection.label ? ` · ${connection.label}` : ''}</ItemDescription></ItemContent>
                    {connection.copyValue && <ItemActions><Button type="button" variant="ghost" size="icon" className="size-7" title="Copy internal hostname" onClick={() => { void navigator.clipboard.writeText(connection.copyValue!); toast.success('Copied internal hostname to clipboard'); }}><Copy className="size-3.5" /><span className="sr-only">Copy internal hostname</span></Button></ItemActions>}
                </Item>
            ))}</ItemGroup>}
        </div>
    );
    const externalDomain = app?.appDomains[0];
    const externalUrl = externalDomain ? `${externalDomain.useSsl ? 'https' : 'http'}://${externalDomain.hostname}` : undefined;
    return (
        <Sheet open={open} onOpenChange={onOpenChange}>
            <SheetContent side="right" className="flex w-full flex-col p-0 sm:max-w-md">
                <Tabs defaultValue="overview" className="flex min-h-0 flex-1 flex-col">
                    <SheetHeader className="gap-4 border-b p-6 pb-0 pr-12">
                        <div className="flex items-start gap-3">
                            <div className={cn('flex size-10 shrink-0 items-center justify-center rounded-lg ring-1', isApp ? 'bg-qs-500/10 text-qs-600 ring-qs-500/30' : 'bg-violet-500/15 text-violet-600 ring-violet-500/30')}>
                                {isApp ? <Boxes className="size-5" /> : <Bot className="size-5" />}
                            </div>
