...
export default function ChooseTemplateDialog({
    ...
    projectId: string;
    templateType: 'database' | 'template' | 'agent-template' | undefined;
    onClose: () => void;
}) {

    const [isOpen, setIsOpen] = useState<boolean>(false);
    const [chosenAppTemplate, setChosenAppTemplate] = useState<AppTemplateModel | undefined>(undefined);
    const [chosenAgentTemplate, setChosenAgentTemplate] = useState<AgentTemplateModel | undefined>(undefined);
    ...
    return (
        <>
            ...
            <CreateTemplateAgentSetupDialog agentTemplate={chosenAgentTemplate} projectId={projectId}
                dialogClosed={() => {
                    setChosenAgentTemplate(undefined);
                    onClose();
                }} />
            <Dialog open={!!isOpen} onOpenChange={(isOpened) => {
                ...
                <DialogContent className="sm:max-w-[1000px]">
                    <DialogHeader>
                        <DialogTitle>Create {templateType === 'database' ? 'Database' : templateType === 'agent-template' ? 'Agent' : 'App'} from Template</DialogTitle>
                        <DialogDescription>
                                ...
                                return (
                                    <div key={template.name}
                                        ...
                                        onClick={() => {
                                            setIsOpen(false);
                                            if (templateType === 'agent-template') {
                                                setChosenAgentTemplate(template as AgentTemplateModel);
                                            } else {
