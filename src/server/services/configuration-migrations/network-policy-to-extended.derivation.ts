...
function normalizePolicy(raw: string): 'ALLOW_ALL' | 'INTERNET_ONLY' | 'NAMESPACE_ONLY' | 'DENY_ALL' {
 ...
 * node ports.
 */
export function deriveExtendedConfiguration(
    app: NetworkPolicyMigrationAppView,
    peers: NetworkPolicyMigrationPeerView[],
): NetworkPolicyMigrationDerivation {
    const ingressPolicy = normalizePolicy(app.ingressNetworkPolicy);
    const egressPolicy = normalizePolicy(app.egressNetworkPolicy);
