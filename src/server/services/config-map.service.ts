...
class ConfigMapService {
    ...
    private async getConfigMapsForApp(projectId: string, appId: string) {
        const configMaps = await k3s.core.listNamespacedConfigMap({ namespace: projectId });

        return configMaps.items.filter(cm => {
    ...
    async getExistingConfigMap(namespace: string, configMapName: string) {
        const configMaps = await k3s.core.listNamespacedConfigMap({ namespace: namespace });
        return configMaps.items.find(cm => cm.metadata?.name === configMapName);
    }
    ...
    async createOrUpdateConfigMap(namespace: string, configMapManifest: k8s.V1ConfigMap) {
        ...
        if (!!existingConfigMaps) {
            await k3s.core.replaceNamespacedConfigMap({ name: currentConfigMapName, namespace: namespace, body: configMapManifest });
        } else {
            await k3s.core.createNamespacedConfigMap({ namespace: namespace, body: configMapManifest });
        }
    }
    ...
    async deleteUnusedConfigMaps(app: AppExtendedModel) {
        ...
        for (const cm of existingConfigMaps) {
            if (!app.appFileMounts.some(fm => KubeObjectNameUtils.getConfigMapName(fm.id) === cm.metadata?.name)) {
                await k3s.core.deleteNamespacedConfigMap({ name: cm.metadata!.name!, namespace: app.projectId });
            }
        }
    }
    ...
    async deleteConfigMapIfExists(namespace: string, configMapName: string) {
        ...
        if (!!existingConfigMap) {
            await k3s.core.deleteNamespacedConfigMap({ name: configMapName, namespace: namespace });
        }
    }
}
