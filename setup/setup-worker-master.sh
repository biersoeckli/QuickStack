...
# TODO: Exact server version has to be an input, otherwise adding new master fails.
# curl -sfL https://get.quickstack.dev/setup-master.sh | K3S_URL=<https://IP-ADDRESS-OR-HOSTNAME-OF-MASTERNODE:6443> JOIN_TOKEN=<TOKEN> sh -
...
# Installation of k3s as an additional server/master node (joins existing embedded etcd cluster)
echo "Installing k3s as an additional master node with --flannel-iface=$selected_iface"
curl -sfL https://get.k3s.io | INSTALL_K3S_VERSION="$K3S_VERSION" K3S_TOKEN="${JOIN_TOKEN}" sh -s - server \
    --server "${K3S_URL}" \
    --flannel-iface="$selected_iface"

echo ""
echo "-----------------------------------------------------------------------------------------------------------"
echo "* Master Node Setup completed. It might take a few minutes until the node is visible in the QuickStack settings. *"
echo "-----------------------------------------------------------------------------------------------------------"
echo ""